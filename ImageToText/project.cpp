...
void Project::AddText(std::unique_ptr<TextBox> text)
{
	...
	guid0 = guid + 1;
	guids.insert(guid);
	appends.push_back(std::move(text));
}

void Project::Save(const std::filesystem::path& path)
{
	auto widgets = UIWidgets();
	auto parent = FindNode(widgets->Children(), [this](INode* node) { return GetGuid(node) == appends[0]->parent; });
	auto arr = parent->Find(503)->Children()[0];
	auto data = arr->Get<std::vector<uint8_t>>();
	std::vector<VarInt> datas;

	for (auto& text : appends)
	{
		text->Create(widgets);
...
void TextBox::Create(INode* widgets)
{
	...
	obj->CreateNode(501, true)->CreateNode(501, guid);
	With(obj->CreateNode(502, true)->CreateNode(502), [this](INode* node)
		{
	...
	With(obj->CreateNode(505, true), [this](INode* node)
		{
			...
			With(node->CreateNode(505), [this](INode* node)
				{
					With(node->CreateNode(14), [this](INode* node)
						{
